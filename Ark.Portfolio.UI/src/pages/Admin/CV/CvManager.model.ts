...
export type CvTab = 'profile' | 'experience' | 'education' | 'skills' | 'ai-settings';

export const useCvManagerModel = () => {
    const [cvData, setCvData] = useState<AdminCvDto | null>(null);
    ...
    const fetchCv = useCallback(async () => {
        setIsLoading(true);
        try {
            const response = await axios.get(`${API_URL}/cv`, getAuthHeaders());
            setCvData(response.data);
        } catch (err) {
            showError('Failed to fetch CV data');
        } finally {
            setIsLoading(false);
        }
    }, []);
    ...
    const fetchCategories = useCallback(async () => {
        try {
            const response = await axios.get(`${API_URL}/cv/categories`, getAuthHeaders());
    ...
    useEffect(() => {
        ...
        fetchCategories();
        fetchAiSettings();
    }, [fetchCv, fetchCategories, fetchAiSettings]);
    ...
    const updateProfile = async (profile: AdminProfileDto) => {
        setIsSaving(true);
        try {
            await axios.put(`${API_URL}/cv/profile`, profile, getAuthHeaders());
            await fetchCv();
    ...
    const createExperience = async (experience: AdminExperienceDto) => {
        setIsSaving(true);
        try {
            await axios.post(`${API_URL}/cv/experience`, experience, getAuthHeaders());
            await fetchCv();
    ...
    const updateExperience = async (id: number, experience: AdminExperienceDto) => {
        setIsSaving(true);
        try {
            await axios.put(`${API_URL}/cv/experience/${id}`, experience, getAuthHeaders());
            await fetchCv();
    ...
    const deleteExperience = async (id: number) => {
        ...
        try {
            await axios.delete(`${API_URL}/cv/experience/${id}`, getAuthHeaders());
            await fetchCv();
    ...
    const createEducation = async (education: AdminEducationDto) => {
        setIsSaving(true);
        try {
            await axios.post(`${API_URL}/cv/education`, education, getAuthHeaders());
            await fetchCv();
    ...
    const updateEducation = async (id: number, education: AdminEducationDto) => {
        setIsSaving(true);
        try {
            await axios.put(`${API_URL}/cv/education/${id}`, education, getAuthHeaders());
            await fetchCv();
    ...
    /**
     * Reorder education entries by updating their display order.
    ...
    const createSkill = async (skill: AdminSkillDto) => {
        setIsSaving(true);
        try {
            await axios.post(`${API_URL}/cv/skill`, skill, getAuthHeaders());
            await fetchCv();
    ...
    const updateSkill = async (id: number, skill: AdminSkillDto) => {
        setIsSaving(true);
        try {
            await axios.put(`${API_URL}/cv/skill/${id}`, skill, getAuthHeaders());
            await fetchCv();
            await fetchCategories();
    ...
    const deleteSkill = async (id: number) => {
        ...
        try {
            await axios.delete(`${API_URL}/cv/skill/${id}`, getAuthHeaders());
            await fetchCv();
    ...
    const createCategory = async (category: SkillCategoryDto) => {
        setIsSaving(true);
        try {
            await axios.post(`${API_URL}/cv/category`, category, getAuthHeaders());
            await fetchCategories();
    ...
    const updateCategory = async (id: number, category: SkillCategoryDto) => {
        setIsSaving(true);
        try {
            await axios.put(`${API_URL}/cv/category/${id}`, category, getAuthHeaders());
            await fetchCategories();
    ...
    const deleteCategory = async (id: number) => {
        ...
        try {
            await axios.delete(`${API_URL}/cv/category/${id}`, getAuthHeaders());
            await fetchCategories();
            await fetchCv();
    ...
    const organizeSkillsWithAi = async () => {
        ...
        try {
            const response = await axios.post(`${API_URL}/ai/organize-skills`, {
                skills: cvData.skills.map(s => ({ id: s.id, name: s.name, level: s.level }))
            }, getAuthHeaders());

            if (response.data.categories?.length) {
                showSuccess(`AI suggested ${response.data.categories.length} categories`);
                // Could auto-create categories here or show preview
            }
            return response.data;
        } catch (err) {
