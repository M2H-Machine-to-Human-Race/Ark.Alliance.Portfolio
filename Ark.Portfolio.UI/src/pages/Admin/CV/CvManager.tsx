...
import { useCvManagerModel, CvTab } from './CvManager.model';
...
import './CvManager.styles.css';

// ============================================
// Tab Navigation
// ============================================

const tabs: { key: CvTab; label: string; icon: React.ReactNode }[] = [
...
export const CvManager: React.FC = () => {
    const vm = useCvManagerModel();

    if (vm.isLoading) {
        return (
            <AdminLayout title="CV Manager">
                <div className="cv-loading">Loading CV data...</div>
            </AdminLayout>
        );
    }
...
const ProfileTab: React.FC<{ vm: ReturnType<typeof useCvManagerModel> }> = ({ vm }) => {
    const [formData, setFormData] = useState({
        firstName: vm.cvData?.profile?.firstName || '',
        lastName: vm.cvData?.profile?.lastName || '',
        title: vm.cvData?.profile?.title || '',
        overview: vm.cvData?.profile?.overview || '',
        email: vm.cvData?.profile?.email || '',
        linkedinUrl: vm.cvData?.profile?.linkedinUrl || '',
        githubUrl: vm.cvData?.profile?.githubUrl || ''
    });

    useEffect(() => {
        if (vm.cvData?.profile) {
            setFormData({
                firstName: vm.cvData.profile.firstName || '',
                lastName: vm.cvData.profile.lastName || '',
                title: vm.cvData.profile.title || '',
...
const ExperienceTab: React.FC<{ vm: ReturnType<typeof useCvManagerModel> }> = ({ vm }) => {
...
const EducationTab: React.FC<{ vm: ReturnType<typeof useCvManagerModel> }> = ({ vm }) => {
    const [isEditing, setIsEditing] = useState(false);
...
const SkillsTab: React.FC<{ vm: ReturnType<typeof useCvManagerModel> }> = ({ vm }) => {
    ...
    const skillsByCategory = (vm.cvData?.skills || []).reduce((acc, skill) => {
...
const AiSettingsTab: React.FC<{ vm: ReturnType<typeof useCvManagerModel> }> = ({ vm }) => {
